...
export function drawWatchTelemetryOverlay(ctx, world, spectatorView = 'full') {
  ...
  for (const team of teams) {
    ...
    const population = getPopulationState(world, team);
    const populationText = `${population.living}/${getCap(world, team)} pop${population.queued > 0 ? ` (+${population.queued} queued)` : ''}`;
    ctx.fillStyle = '#e8e8ee'; ctx.font = '12px monospace'; ctx.fillText(`${state.gold} gold`, textX, 35); ctx.fillText(populationText, textX, 51); ctx.fillText(`M${c.miner} W${c.warrior} A${c.archer} S${c.structures} T${c.turrets}`, textX, 67); ctx.restore();
  }
}

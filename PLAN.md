...
**Included:** replace the current dark full-map Player-vs-AI fog presentation with a light, translucent combat-area fog. Clear the new layer through the exact existing player team-vision sources: current vision, renderer-owned sustained/fading vision samples, and combat-reveal sources. Keep hidden mobile enemy, structure/base, Raven, and projectile visibility semantics unchanged. Limit the fog layer to the playable map, never HUD/menu/build controls. Watch-AI presentation remains unchanged.

**Approved extension — 2026-08-28:** increase the pale fog opacity by 75% (from `0.48` to `0.84`), confine it to the lower map/combat half so sky and mountains remain un-fogged, and draw enemy structures, turrets, and core **beneath** the fog layer: their known locations remain visible, but they receive the same visual fog treatment until player vision clears them.

**Deliberate exclusions:** simulation vision, AI knowledge/memory, combat/reveal rules, team-vision radii/timing, Watch-AI fog styling, camera behavior, game balance, deployment/push, and generated screenshot versioning.

**Evidence gates:** a red→green Canvas rendering regression proves a pale fog fill, isolated `destination-out` vision clearing with overlapping-source union, map-only bounds, and the Player-only feathered edge. Retain player-fog, sustained-vision, and fog-performance checks. Use a cache-busted real browser Player-vs-AI fixture to inspect hidden/revealed terrain and retain normal hidden-mobile behavior. Hash the changed LAN-served module against the root worktree before sharing the review URL. Make a focused local commit after validation; do not push.

**Active refinement (owner-approved):** halve the Player-vs-AI veil again to `rgba(225, 232, 240, 0.075)` so terrain/backdrop and static objectives show naturally through it. Keep the Player-only radial vision feather, but scale its world-space width and the fog’s combat-area top boundary with camera zoom; retain the build-control bottom bound. Add soft vertical fades at the fog’s top and bottom limits. Increase Raven flying vision by 50%, from `375` to `562.5`, while leaving its enemy-base reveal radius/timing unchanged. Rendering stays presentation-only apart from the explicitly requested Raven vision radius; do not alter spectator fog, AI policy, other unit sight, combat reveal, or queue mechanics. The Canvas regression must assert 0.075 color, zoom-projected top/feather, radial/vertical gradients, and Raven radius; browser evidence must show zoom-scaled light fog, soft top/bottom/vision edges, intact sky/HUD, and normal hidden-mobile behavior. No commit/push until owner LAN review.

**Performance extension (owner-approved):** retain the soft Player vision edge and vertical fog fades, but bound renderer-owned vision-memory history to the newest 12 moving samples per source and refresh unchanged source samples in place. This must leave simulation/team vision, AI knowledge, combat reveal, Raven rules, and all fog visibility semantics unchanged. Evidence requires a real browser stress comparison showing the retained 100-source moving-history case stays at 60 FPS without the former radial-feather p95 hitch, alongside retained visual/browser regression checks. No commit/push until owner LAN review.

**Implemented / paused after local integration:** Player-vs-AI now uses the pale `rgba(225, 232, 240, 0.075)` lower-combat fog, a zoom-scaled radial clearance feather, and soft vertical top/bottom fades; sky/mountains, HUD, and Watch-AI fog remain unchanged. Flying Raven vision is `562.5` while its base-reveal radius/timing stay unchanged. Enemy static structures/turrets/core retain restrained known-location silhouettes without exposing mobile enemy units or live core state. Renderer-owned vision memory refreshes stationary sources and caps moving trails at 12 samples per source, reducing the reproduced radial-feather stress P95 from `33.4ms` at 4,800 samples to `16.8ms` at the bounded 1,200-sample case. Focused render/player/sustain/viewport-performance checks, browser hidden/revealed proof, LAN hash verification, and `git diff --check` passed. Owner still observes residual subjective lag; the agreed next production-faithful profiling/offscreen-culling investigation is deliberately paused. This approved fog scope is committed locally on `main`; it has not been pushed to `origin`. Generated screenshots and diagnostic artifacts remain local-only.

...
await evaluate(`window.__telemetryText = []; const originalFillText = CanvasRenderingContext2D.prototype.fillText; CanvasRenderingContext2D.prototype.fillText = function(text, ...rest) { window.__telemetryText.push(String(text)); return originalFillText.call(this, text, ...rest); }; window.__resetMatch('medium'); window.__world.teams.player.losses = 2; window.__world.teams.ai.losses = 5; window.__world.matchElapsedTime = 222.9;`);
await new Promise((resolve) => setTimeout(resolve, 100));
const telemetry = await evaluate(`window.__telemetryText.slice(-500)`);
for (const label of ['BLUE 5', '2 RED', '03:42']) if (!telemetry.includes(label)) throw new Error(`Player-vs-AI telemetry missing ${label}: ${JSON.stringify(telemetry.slice(-80))}`);
const goldLabel = telemetry.find((text) => /^\d+ gold$/.test(text));
if (!goldLabel) throw new Error(`Player-vs-AI gold differential missing: ${JSON.stringify(telemetry.slice(-80))}`);
await click(1307, 24);
...
console.log(JSON.stringify({ appUrl: APP_URL, telemetry: ['BLUE 5', '03:42', '2 RED', goldLabel], pause: 'passed', zoom: { zoomBefore, zoomAfter }, screenshotPath }, null, 2));

...
const APP_URL = process.env.APP_URL || `http://192.168.0.83:8813/?victory-update-log=${Date.now()}`;
...
await evaluate(`window.__resetMatch('medium'); window.__world.matchState = 'won'; window.__forceTicks(1);`);
const exit = await evaluate(`(() => { const r = document.querySelector('#game').getBoundingClientRect(); return { x: r.left + (700 / 1400) * r.width, y: r.top + (389 / 540) * r.height }; })()`);
for (const type of ['mousePressed', 'mouseReleased']) await send('Input.dispatchMouseEvent', { type, x: exit.x, y: exit.y, button: 'left', buttons: type === 'mousePressed' ? 1 : 0, clickCount: 1 });
await new Promise((resolve) => setTimeout(resolve, 100));
const exitState = await evaluate(`({ matchState: window.__world.matchState, menuScreen: window.__uiState.menuScreen })`);
if (exitState.matchState !== 'menu' || exitState.menuScreen !== 'main') throw new Error(`Exit to Menu click failed: ${JSON.stringify(exitState)}`);
await evaluate(`window.__uiState.menuScreen = 'updates'; window.__forceTicks(1);`);
const screenshot = await send('Page.captureScreenshot', { format: 'png' }); const screenshotPath = join('artifacts', 'screenshots', 'victory-update-log-browser.png'); mkdirSync(join('artifacts', 'screenshots'), { recursive: true }); writeFileSync(screenshotPath, screenshot.data, 'base64');
ws.close(); if (errors.length) throw new Error(`Browser errors: ${JSON.stringify(errors)}`); console.log(JSON.stringify({ appUrl: APP_URL, exitState, screenshotPath }, null, 2));
